...
public class VectorEx {
	...
	public static void main(String[] args) {
		...
		System.out.printf("size : %d, capacity : %d\n", v.size(), v.capacity());			// size = 갯수 capacity = 버퍼 사이즈
		// 1부터 10까지 값을 담아 봅니다
		for(int i = 1; i <= 10; i++) {
			v.addElement(i);																// 추가
		...
		System.out.printf("%d번 인덱스의 객체 %d\n", 7, value);
		
		// 객체 인덱스 검색 indexOf
		System.out.println("객체 7의 인덱스 ? " + v.indexOf(7));
		
		// 없는 객체의 검색 결과
		System.out.println("없는 객체의 인덱스  ? " + v.indexOf(0));								// -1로 출력됨
...
//		v2.addElement(Boolean.valueOf(true));												// Boolean은 Number의 자식이 아님
