...
//		basicSearch();
	...
	private static void customSearch() {
		...
		Arrays.sort(members);				// 데이터 정렬	
		System.out.println("정렬된 배열 : " + Arrays.toString(members));
		int index = Arrays.binarySearch(members,
				new Member("장길산"));		// 사용자 정의 객체의 검색 시에는 객체를 넘겨주어 해당 객체와 같은 지를 비교해주어야 한다
	...
	private static void basicSearch() {
		...
		Arrays.sort(num);			// 바이너리 검색 이전에 반드시 정렬이 되어 있어야 한다
	...
	// 역순 정렬
	// 내림차순(Descending(DESC) : 큰 값 -> 작은 값 순)
	private static void basicSortDescendingTest() {
		Integer[] scores = {80, 50, 75, 90, 70, 88, 76};				// int -> integer
