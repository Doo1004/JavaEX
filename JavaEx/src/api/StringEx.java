...
//		stringMethods();
	...
	private static void stringBufferEx() {
		// 복잡한 문자열을 가공할 때 매번 새로운 문자열 객체 생성 -> 내부에 버퍼(임시 저장공간)를 가진 StringBuffer를 활용하면 메모리 관리가 용이할 수 있다
	...
	private static void stringMethods() {
		...
		// 검색 관련 ㅣ 문자, 문자열의 위치를 찾을 때
		System.out.println("5번 인덱스의 문자 : " + s.charAt(5));
		
		// 중요! indexOf
		System.out.println("Fun 문자열의 위치 인덱스 : " + s.indexOf("Fun"));		// 검색어 대소문자를 구분하기 때문에 확실하게 입력할 것
		System.out.println("fun 문자열의 위치 인덱스 : " + s.indexOf("fun"));		// 반환값이 -1 => 해당 값을 찾을 수 없음(중요)
		System.out.println("m의 위치 인덱스 : " + s.indexOf("m"));				// 11
		System.out.println("m의 역방향 위치 인덱스 : " + s.lastIndexOf("m"));		// 12
		System.out.println("m의 위치 인덱스 2nd : " + s.indexOf("m", 12));		// 검색 시작 위치가 12번 인덱스로부터 시작됨
		...
		System.out.println("원본데이터 : " + s);								// 원본 데이터는 변하지 않음
		...
		// 문자열 분리 : split
		String[] chunks = s.replace("?",  "").split(" ");										// 문자열 s를 공백을 기준으로 분리 -> 문자열 배열
